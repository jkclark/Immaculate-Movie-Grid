...
const EmptyGameSquare: React.FC<EmptyGameSquare> = ({ row, col, clickHandler }) => {
  ...
  return (
    <div
      ...
      className={`w-full h-full border border-gray-600 ${row === selectedRow && col === selectedCol && "bg-sky-100"} ${!gameOver ? "hover:bg-sky-100 hover:cursor-pointer" : "hover:cursor-default"}`}
    ></div>
  );
};
